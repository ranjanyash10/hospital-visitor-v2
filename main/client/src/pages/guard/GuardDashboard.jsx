...
const GuardDashboard = () => {
    ...
    const fetchStats = async () => {
        try {
            ...
            setSlips(slipsRes.data.slips || slipsRes.data || []);
        } catch (err) {
                    ...
                    <div className="w-full max-w-2xl text-center">
                        ...
                        <div className="bg-white/70 backdrop-blur-lg rounded-3xl p-6 md:p-8 shadow-xl shadow-emerald-500/10 border border-emerald-100">
                            <div className="grid grid-cols-2 gap-4 md:gap-6">
                                <div className="bg-white rounded-2xl p-4 md:p-5 text-left shadow-sm">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5">
                                        <User size={10} /> Patient
                                    </p>
                                    <p className="text-lg md:text-2xl font-black text-slate-800 tracking-tight">{successData.patient_name}</p>
                                </div>
                                <div className="bg-white rounded-2xl p-4 md:p-5 text-left shadow-sm">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5">
                                        <Building2 size={10} /> Room
                                    </p>
                                    <p className="text-lg md:text-2xl font-black text-slate-800 tracking-tight">{successData.room_number}</p>
                                </div>
                                <div className="bg-white rounded-2xl p-4 md:p-5 text-left shadow-sm">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5">
                                        <Activity size={10} /> Bed
                                    </p>
                                    <p className="text-lg md:text-2xl font-black text-slate-800 tracking-tight">{successData.bed_number}</p>
                                </div>
                                <div className="bg-white rounded-2xl p-4 md:p-5 text-left shadow-sm">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2">
                                        📱 Mobile
                                    </p>
                                    <p className="text-lg md:text-2xl font-black text-slate-800 tracking-tight">{successData.masked_mobile}</p>
                                </div>
                            </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            {/* Header */}
            <header className="sticky top-0 z-50 bg-white border-b border-slate-100 px-4 md:px-8 py-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-base font-black text-slate-800 tracking-tight">Guard Station</h1>
                            <p className="text-[8px] font-bold text-blue-600 uppercase tracking-[0.25em]">Action Care Hospital</p>
                        </div>
                    </div>
                ...
                <main className="flex-1 flex flex-col items-center p-6 md:p-8 overflow-y-auto">
                    {mode === 'LOADING' ? (
                        ...
                        <>
                            ...
                            {/* QR Code */}
                            <div className="bg-white rounded-3xl shadow-2xl shadow-slate-300/30 p-6 md:p-10 mb-6">
                                {qrToken ? (
                                    <QRCode
                                        value={qrToken}
                                        size={280}
                                    ...
                                    <div className="w-[280px] h-[280px] bg-slate-100 rounded-xl flex items-center justify-center">
                            ...
                            {visitorUrl && (
                                <div className="text-center max-w-md">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Or visit this link</p>
                                    <a
                                        href={visitorUrl}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="block text-xs font-mono text-blue-600 bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-100 break-all hover:bg-blue-100 hover:border-blue-200 transition-colors underline decoration-blue-300 underline-offset-2"
                                    >
                                        {visitorUrl}
                                    </a>
                                </div>
                            ...
                            <div className="mt-8 flex gap-4">
                                <div className="px-5 py-3 bg-white rounded-2xl border border-slate-100 shadow-sm text-center">
                                    <p className="text-2xl font-black text-blue-600">{stats.activeSlips}</p>
                                ...
                                {stats.lockdown && (
                                    <div className="px-5 py-3 bg-red-50 rounded-2xl border border-red-100 text-center">
                                        ...
                                        <table className="w-full text-left">
                                            ...
                                            <tbody>
                                                {activeVisitors.map((slip, i) => (
                                                    <tr key={slip.id} className={`border-b border-slate-50 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-blue-50/50 transition-colors`}>
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${slip.visitor_count > 1 ? 'bg-indigo-100 text-indigo-700' : 'text-slate-500'}`}>
                ...
                {showPanel && (
                    <aside className="w-80 bg-white border-l border-slate-100 overflow-y-auto p-5 animate-in slide-in-from-right duration-300">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider mb-4 flex items-center gap-2">
                            <Database size={14} className="text-blue-600" /> Recent Slips
                        </h3>
                        <button onClick={fetchStats} className="mb-4 text-xs font-bold text-blue-600 flex items-center gap-1 hover:underline">
                            <RefreshCw size={10} /> Refresh
                        </button>
                                ...
                                <div key={slip.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    <div className="flex items-center justify-between mb-1">
                                        ...
                                        <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${slip.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' :
                                            slip.status === 'VISITING' ? 'bg-blue-100 text-blue-700' :
